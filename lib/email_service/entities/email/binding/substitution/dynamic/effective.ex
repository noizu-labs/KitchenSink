...
defmodule Noizu.EmailService.Email.Binding.Substitution.Dynamic.Effective do
  ...
  def insert_path(blob, [h], selector, p) do
    ...
    block = case h do
             {:at, v} ->
               ...
               index_size = get_in(block, parent_index_size_path)
               cond do
                 v + 1 >= index_size -> put_in(block, parent_index_size_path, v + 1)
                 :else -> block
               end
             _else -> block
           end
  ...
  # interstitial_map
  #----------------------
  def interstitial_map(%__MODULE__{} = this, _state, _context, _options) do
    bind = this.bind
           |> Enum.uniq()
           |> Enum.sort_by(&(&1.selector < &1.selector))

    _book_keeping = Enum.reduce(bind, %{}, fn(selector, acc) ->
  ...
  defp build_output_inner({_k,v}, _blob, state, context, options) do
    cond do
      v.type == :copy ->
  ...
  #
  #----------------------
  def new(%Section{} = section, state, _context, _options) do
    {%__MODULE__{bind:  Enum.uniq(section.bind)}, state}
  end

  def new(%Formula.IfThen{condition_clause: _formula} = section, state, _context, _options) do
  ...
  def new(%Formula.Each{} = _section, state, _context, _options) do
    {%__MODULE__{}, state}
  end

  def new(bind, state, _context, _options) when is_list(bind) do
    bind =  bind
            |> Enum.uniq()
    {%__MODULE__{bind: bind}, state}
  end

  def new(nil, state, _context, _options) do
  ...
  #
  #----------------------
  def set_wildcard_hint(%__MODULE__{} = this, %Selector{} = selector, :list, {index, value}, state, context, _options) do
  ...
  #
  #----------------------
  def set_wildcard_hint(%__MODULE__{} = this, %Selector{} = selector, :kv, {key, value}, state, context, _options) do
    {r,s} = Noizu.RuleEngine.StateProtocol.get!(state, :wildcards, context)
  ...
  #
  #----------------------
  def clear_wildcard_hint(%__MODULE__{} = this, %Selector{} = selector, _type, state, context, _options) do
    {r,s} = Noizu.RuleEngine.StateProtocol.get!(state, :wildcards, context)
  ...
  #
  #----------------------
  def merge(%__MODULE__{} = bind_a, %__MODULE__{} = bind_b, state, _context, _options) do
    r = cond do
          bind_a.meta[:wildcard] ->
            {ws,wr} = bind_a.meta[:wildcard]
            wsl = length(ws)
            _r = Enum.map(bind_b.bind || [], fn(b_s) ->
              cond do
  ...
  def merge(%__MODULE__{} = bind_a, nil, state, _context, _options), do: {bind_a, state}
  def merge(nil, %__MODULE__{} = bind_b, state, _context, _options) do
